...
@doc raw"""
    UnscentedKalmanFilter(model::SimModelODE; <keyword arguments>)

Construct an unscented Kalman Filter with the [`SimModelODE`](@ref) `model`.
...
- `model::SimModelODE` : (deterministic) model for the estimations.
- `i_ym=1:model.ny` : `model` output indices that are measured ``\mathbf{y^m}``, the rest 
    are unmeasured ``\mathbf{y^u}``.
- `σP_0=fill(1/model.nx,model.nx)` or *`sigmaP_0`* : main diagonal of the initial estimate
    covariance ``\mathbf{P}(0)``, specified as a standard deviation vector.
...
function UnscentedKalmanFilter(
    ...
    β = beta,
    κ = kappa,
) where {NT<:Real, SM<:SimModelODE{NT}}
    # estimated covariances matrices (variance = σ²) :
    P̂_0 = Diagonal([σP_0; σPint_u_0; σPint_ym_0].^2)
    Q̂   = Diagonal([σQ;  σQint_u;  σQint_ym ].^2)
    R̂   = Diagonal([σR;].^2)
...
\begin{aligned}
    ...
    \mathbf{Ŷ^m}(k)     &= \bigg[\begin{matrix} \mathbf{ĥ^m}\Big( \mathbf{X̂}_{k-1}^{1}(k), \mathbf{d}(k) \Big) & \mathbf{ĥ^m}\Big( \mathbf{X̂}_{k-1}^{2}(k), \mathbf{d}(k) \Big) & \cdots & \mathbf{ĥ^m}\Big( \mathbf{X̂}_{k-1}^{n_σ}(k), \mathbf{d}(k) \Big) \end{matrix}\bigg] \\
...
@doc raw"""
    ExtendedKalmanFilter(model::SimModelODE; <keyword arguments>)

Construct an extended Kalman Filter with the [`SimModelODE`](@ref) `model`.
